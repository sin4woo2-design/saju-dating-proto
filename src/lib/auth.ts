...
function cleanAuthParams(url: URL) {
  ["auth_status", "provider", "user_id", "user_name", "user_email", "avatar_url", "access_token", "refresh_token", "error", "error_description", "code"].forEach((key) => {
...
async function fetchSupabaseUser(accessToken: string) {
  ...
  if (!apiUrl || !publicKey) return null;
...
export async function consumeOAuthCallback(): Promise<{ session?: AuthSession; error?: string; success?: string }> {
  ...
  if (!accessToken && !error) {
    ...
    if (storedSuccess && storedSession) return { session: storedSession, success: storedSuccess };
    if (storedSession) return { session: storedSession };
    return {};
  }
  ...
  if ((!user || !provider) && accessToken) {
    ...
    provider = providerValue === "google" ? providerValue : provider;
...
export function startOAuthLogin(provider: AuthProvider) {
  ...
  const apiUrl = supabaseUrl();
  const publicKey = supabasePublicKey();
  if (!apiUrl || !publicKey) return { ok: false, reason: "MISSING_CONFIG" as const };
