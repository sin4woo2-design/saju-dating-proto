...
`이 글은 과장된 표현 없이, 일상에서 바로 실천 가능한 방법만 간단히 정리했습니다.\n\n` +
`왜 이 주제가 중요한가\n` +
...
function fitToAround2000(text) {
  ...
  const addons = [
    "\n\n추가 정리: 완벽한 계획보다 매일 반복 가능한 작은 행동이 훨씬 효과적입니다.",
    "\n\n추가 정리: 하루 놓쳐도 실패가 아닙니다. 다음 날 같은 시간에 다시 시작하면 충분합니다.",
    "\n\n추가 정리: 몸 상태에 따라 강도를 조절하는 것이 꾸준함을 지키는 가장 현실적인 방법입니다.",
    "\n\n추가 정리: 기록은 길게 쓰지 않아도 됩니다. 실행 여부만 체크해도 변화가 보입니다.",
  ];

  let out = text;
  let i = 0;
  while (out.length < min) {
    out += addons[i % addons.length];
    i += 1;
  }
